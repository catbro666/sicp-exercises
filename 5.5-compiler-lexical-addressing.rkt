...
(define (compile exp target linkage env)
  (cond ((self-evaluating? exp)
        ...
        ((open-code-application-arbi? exp env)
         (compile-open-code-arbi exp target linkage env))
        ((open-code-application? exp env)
...
(define (spread-arguments argl env)
  (let ((operand-code1 (compile (car argl) 'arg1 'next env))
        (operand-code2 (compile (cadr argl) 'arg2 'next env)))
    (list operand-code1 operand-code2)))

(define (compile-open-code exp target linkage env)
  (let ((op (operator exp))
        (args (spread-arguments (operands exp) env)))
    (end-with-linkage
     linkage
     (preserving
      '(env)
...
(define (compile-open-code-arbi exp target linkage env)
  (let ((op (operator exp))
      ...
      (cond
       ...
       (else
        (end-with-linkage
         ...
         (preserving
          '(env)
          (compile (car args) 'arg1 'next env)
          (compile-open-code-rest op (cdr args) target env))))))))

(define (compile-open-code-rest operator operands target env)
    ...
    (define (scan displacement vars)
      (cond ((null? vars)
             (env-loop (+ frame-num 1)
                       (enclosing-cenv cenv)))
            ((eq? var (car vars))
             (make-lexical-address frame-num displacement))
            (else
             (scan (+ 1 displacement) (cdr vars)))))
    (if (eq? cenv the-empty-cenv)
        'not-found
        (scan 0 (cenv-first-frame cenv))))
  (env-loop 0 cenv))

(provide compile registers-needed registers-modified statements the-empty-cenv)
